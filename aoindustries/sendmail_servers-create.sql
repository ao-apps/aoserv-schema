...
create table sendmail_servers (
  ...
  max_queue_children       integer default 100
    check (
      max_queue_children is null
      or max_queue_children > 0
    ),
  nice_queue_run           integer default 10
    check (
      nice_queue_run is null
      or nice_queue_run > 0
    ),
  delay_la                 integer default 40
    check (
      delay_la is null
      or delay_la > 0
    ),
  queue_la                 integer default 50
    check (
      queue_la is null
      or queue_la > delay_la
    ),
  refuse_la                integer default 80
    check (
      refuse_la is null
      or refuse_la > queue_la
    ),
  max_daemon_children      integer default 1000
    check (
      max_daemon_children is null
      or max_daemon_children > 0
    ),
  bad_rcpt_throttle        integer default 10
    check (
      bad_rcpt_throttle is null
      or bad_rcpt_throttle > 0
    ),
  connection_rate_throttle integer default 100
    check (
      connection_rate_throttle is null
      or connection_rate_throttle > 0
    ),
  max_message_size         integer default 100000000
    check (
      max_message_size is null
      or max_message_size > 0
    ),
  min_free_blocks          integer default 65536
    check (
