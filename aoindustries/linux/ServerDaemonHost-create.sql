create sequence linux."ServerDaemonHost_id_seq" cycle;
grant all    on linux."ServerDaemonHost_id_seq" to aoadmin;
grant select on linux."ServerDaemonHost_id_seq" to aoserv_app;
...
create table linux."ServerDaemonHost" (
  id integer
    default nextval('linux."ServerDaemonHost_id_seq"')
    primary key,
  ao_server integer
    not null,
  "host" text
