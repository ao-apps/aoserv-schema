...
-- httpd_servers
          (select 'public'::name as "schema", 'httpd_servers'::name as "table", count(*) as unused from management."remove-httpd-server"
...
-- mysql_servers
union all (select 'public'::name as "schema", 'mysql.mysql_servers'::name as "table", count(*) as unused from management."remove-mysql-server"
...
-- postgres_servers
union all (select 'public'::name as "schema", 'postgres_servers'::name as "table", count(*) as unused from management."remove-postgresql-server"
           where num_databases=0 or num_users=0)
-- TODO: sendmail_servers without any sendmail_binds
-- TODO: firewalld_zones
union all (select 'public'::name as "schema", 'IPAddress'::name as "table", count(*) as unused from management."release-IPAddress"
           where num_binds=0 and "AO_SERVER" is not null)
-- ssl_certificates
union all (select 'public'::name as "schema", 'pki.ssl_certificates'::name as "table", count(*) as unused from management."remove-ssl-certificate"
