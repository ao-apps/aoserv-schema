...
-- mysql_servers
          (select 'mysql_servers', count(*) as unused from management."remove-mysql-server" where num_databases=0 or num_users=0)
...
-- postgres_servers
union all (select 'postgres_servers', count(*) as unused from management."remove-postgresql-server" where num_databases=0 or num_users=0)
